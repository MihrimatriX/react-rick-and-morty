...
describe("Filter", () => {
	...
	test("renders all filter inputs", () => {
		...
		expect(
			screen.getByLabelText("Search character name")
		).toBeInTheDocument();
	...
	test("calls handleFilter on input change", () => {
		...
		fireEvent.change(screen.getByLabelText("Search character name"), {
			target: { value: "Rick" },
		});
		...
		fireEvent.change(screen.getByLabelText("Select status"), {
			target: { value: "Alive" },
		});
		expect(handleFilter).toHaveBeenCalledWith({
			key: "status",
		...
		fireEvent.change(screen.getByLabelText("Select gender"), {
			target: { value: "Male" },
		});
		expect(handleFilter).toHaveBeenCalledWith({
			key: "gender",
