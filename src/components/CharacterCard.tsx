...
interface CharacterCardProps extends Pick<Character, 'id' | 'img' | 'name' | 'species' | 'status'> {}
...
const CharacterCard = ({ id, img, name, species, status }: CharacterCardProps) => {
  return (
    <Link
      to={`/char/${id}`}
      tabIndex={0}
    ...
    >
      <div className="w-64 flex flex-col text-left m-2 p-0 rounded-2xl bg-card-light dark:bg-card-dark shadow-xl border-2 border-transparent hover:border-accent-light dark:hover:border-accent-dark transition-all duration-300 hover:scale-105 hover:shadow-2xl backdrop-blur-md group cursor-pointer overflow-hidden relative">
        <div className="relative w-full aspect-square overflow-hidden">
          ...
          <span className={`absolute top-2 left-2 px-2 py-1 text-xs rounded-full text-white font-bold shadow ${status === 'Alive' ? 'bg-rick' : status === 'Dead' ? 'bg-evil' : 'bg-portal'}`}>{status}</span>
          <span className={`absolute top-2 right-2 px-2 py-1 text-xs rounded-full text-white font-bold shadow ${species === 'Human' ? 'bg-morty' : species === 'Alien' ? 'bg-evil' : 'bg-portal'}`}>{species}</span>
        </div>
