...
const EpisodeDetail = () => {
  ...
  useEffect(() => {
    ...
    fetchEpisode(id)
      .then(async ep => {
        setEpisode(ep);
        const charUrls = showAll ? ep.characters : ep.characters.slice(0, MAX_VISIBLE);
        const charData = await Promise.all(charUrls.map(fetchCharacter));
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] px-4 animate-fadeIn">
      ...
      <div className="bg-card-light dark:bg-card-dark rounded-lg shadow-lg p-8 max-w-xl w-full transition-colors duration-300">
        <h1 className="text-3xl font-bold text-accent-light dark:text-accent-dark mb-2">{episode.episode} - {episode.name}</h1>
        <p className="text-slate-700 dark:text-slate-200 mb-2"><span className="font-semibold">Yayın Tarihi:</span> {episode.air_date}</p>
        <p className="text-slate-700 dark:text-slate-200 mb-4"><span className="font-semibold">Karakterler:</span></p>
        <div className="flex flex-wrap gap-2 max-h-48 overflow-y-auto scrollbar-thin scrollbar-thumb-rick dark:scrollbar-thumb-morty scrollbar-track-transparent rounded-md pr-1">
          {characters.map(char => (
            ...
            >
              {char.img && <img src={char.img} alt={char.name} className="w-5 h-5 rounded-full object-cover mr-1" />}
              {char.name}
            </Link>
          ))}
