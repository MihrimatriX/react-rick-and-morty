...
import { FaSearch, FaFilter, FaSyncAlt, FaVenusMars, FaHeartbeat } from "react-icons/fa";
  ...
  <section className="w-full flex justify-center mt-8 mb-6 animate-slideDownAndFade">
    ...
    >
      <div className="flex flex-col gap-2 w-full lg:w-auto">
        <label htmlFor="searchBox" className="flex items-center gap-2 text-accent-light dark:text-accent-dark font-bold text-lg transition-colors duration-200 hover:text-rick dark:hover:text-morty">
        ...
        <input
          ...
          onChange={e => handleFilter({ key: "searchBox", value: e.target.value })}
          aria-label="Karakter ismi ara"
        />
      </div>
      <div className="flex flex-col gap-2 w-full lg:w-auto">
        <label htmlFor="status" className="flex items-center gap-2 text-accent-light dark:text-accent-dark font-bold text-lg transition-colors duration-200 hover:text-rick dark:hover:text-morty">
        ...
        <select
          ...
          onChange={e => handleFilter({ key: "status", value: e.target.value })}
          aria-label="Status seç"
        >
          {statusOptions.map(opt => (
            <option key={opt.value} value={opt.value} className="bg-white dark:bg-bg-dark text-slate-900 dark:text-white">{opt.label}</option>
          ))}
        </select>
      </div>
      <div className="flex flex-col gap-2 w-full lg:w-auto">
        <label htmlFor="gender" className="flex items-center gap-2 text-accent-light dark:text-accent-dark font-bold text-lg transition-colors duration-200 hover:text-rick dark:hover:text-morty">
          <FaVenusMars className="text-xl" /> Gender
        </label>
        <select
          id="gender"
          className="px-5 py-3 rounded-xl border-2 border-accent-light/50 dark:border-accent-dark/50 focus:border-rick dark:focus:border-morty focus:ring-4 focus:ring-rick/20 dark:focus:ring-morty/20 bg-white/90 dark:bg-bg-dark/90 text-slate-900 dark:text-white shadow-lg transition-all duration-300 w-full lg:w-40 hover:border-accent-light dark:hover:border-accent-dark hover:shadow-xl hover:scale-[1.02] focus:scale-[1.02] cursor-pointer"
          onChange={e => handleFilter({ key: "gender", value: e.target.value })}
